...
function Find-ProjectsByNameInCommon {
    ...
    @(
        Get-ChildItem -Path $commonRoot -Recurse -Filter "*.csproj" -ErrorAction SilentlyContinue |
            Where-Object { $_.BaseName -eq $name -and $_.FullName -notmatch "\\bin\\" -and $_.FullName -notmatch "\\obj\\" } |
...
# --- Determine solution and search root (ONLY Src\Common) ---
...
try {
    ...
    # --- Find created nupkg (prefer non-symbol packages) ---
    ...
    # If multiple packages exist, pick the newest one
