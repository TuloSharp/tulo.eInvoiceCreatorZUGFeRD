...
public class GenerateFromJsonXmlCiiIntegrationTests
{
    ...
    public void Can_generate_xml_from_json_EXTENDED_Kleinunternehmer_ohneUStId()
    {
        ...
        for (int i = 0; i < expLines.Length; i++)
        {
            ...
            {
                Assert.AreEqual(eTax.RateApplicablePercent.Value, aTax.RateApplicablePercent.Value, $"Line {i + 1} Tax% different.");
            }
        }
    }

    [TestMethod(DisplayName = "Create an xml form ZF_Extended__Abschlagsrechnung_SubInvoiceLine_u_LV_Nr_.json file and verifies it")]
    public void Can_generate_xml_from_json_ZF_Extended__Abschlagsrechnung_SubInvoiceLine_u_LV_Nr_()
    {
        // JSON File find & read
        string baseDir = AppContext.BaseDirectory;
        string jsonPath = Path.Combine(baseDir, "JsonExamples", "ZF_Extended__Abschlagsrechnung_SubInvoiceLine_u_LV_Nr_.json");

        Assert.IsTrue(File.Exists(jsonPath), $"JSON File is not found: {jsonPath}");
        ...
        // Save
        string fileName = $"ZF_Extended__Abschlagsrechnung_SubInvoiceLine_u_LV_Nr_{invoice.InvoiceNumber}_{DateTime.Now:yyyyMMddHHmmssfff}.xml";
        string filePath = Path.Combine(_tempDir, fileName);
        SaveXmlFile(invoice, xml, filePath);
